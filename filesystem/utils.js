 ...
 */
function extractRJCodeFromFolderName(folderName) {
  const match = String(folderName).match(/RJ(\d{6,8})(?!\d)/i);
  ...
  for (const folder of folders) {
    ...
    // eslint-disable-next-line no-await-in-loop
      if ((await fs.promises.stat(absolutePath)).isDirectory()) { // 检查是否为文件夹
          const rjMatch = extractRJCodeFromFolderName(folder);
          if (rjMatch) { // 检查文件夹名称中是否含有RJ号
            // Found a work folder, don't go any deeper.
            yield { absolutePath, relativePath, rootFolderName: rootFolder.name, id: parseInt(rjMatch, 10) };
